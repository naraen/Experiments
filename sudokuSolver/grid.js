(function(){
  ...
  const gridIdxToBoxIdx = ( i ) => parseInt ( gridIdxToRowIdx(i) / 3 ) * 3 + parseInt(gridIdxToColIdx(i) / 3);
  const arrToObject = (arr, acc) =>  arr.reduce( (obj, currVal) => {
      obj[currVal] = '';
      return obj;
    }, acc);
  ...
  function Cell ( gridIdx, grid ) {
    ...
    function removeCandidate_shadow( val ) {
      if (_self.candidates == parseInt(val) ) {
        console.log(tabLevel.join(''), '***', val, 'from', gridIdx, 'Problem.  Halting!');
        isHalted = true;
        return;
      }
  ...
  function Grid( ) {
    ...
    function displayUnsolvedSets() {
      ...
      _self.unsolvedSets.forEach( (set, idx) => {
        ...
        console.log("    ", idx, JSON.stringify(set), '  ||  ', JSON.stringify(candidates));
      });
      console.log("  ]");
    }
    ...
    function getCellsGroupedByUnsolvedNumbers (set) {
      return set.reduce( (acc, cellIdx) =>  {
        thisGrid[cellIdx].candidates.toString()
          .split('')
          .forEach( (n) => {
            if (acc[n] === undefined) {
              acc[n] = [];
    ...
    _self.checkForCorrectness = (isDebug) => {
      ...
      allSets.forEach( (thisSet, setIdx) => {
        var thisSum = thisSet.reduce( (sum, gridIdx) => sum + thisGrid[ gridIdx ].candidates, 0);
        if (thisSum !== 45) {
    ...
    _self.dropSolvedCellFromUnsolvedSets = (cellIdx) => {
      ...
      getSetsContainingACell(cellIdx)
        .forEach( (setIdx) => {
...
//      displayUnsolvedSets();
    ...
    _self.getGridForDisplay = () => {
      ...
      for ( var idx = 0; idx < 81; idx++ ) {
        ...
        rowVals += ' '.repeat(8 - (thisGrid[ idx ].toString().length)) + thisGrid[ idx ];
        rowVals += (idx % 3 === 2) ? '||' : '|'
      }
      rowVals += `\n${'='.repeat(87)}`;
    ...
    _self.useBruteForce = () => {
      var getGridToStash  = () => thisGrid.reduce( (acc, cell) => acc + (!cell.isSolved ? 0 : cell.candidates) , '' );
      ...
      thisGrid[firstUnsolvedPosition]
        .candidates.toString().split('').forEach( (num) => {
          var valueToTry = { gridIdx : firstUnsolvedPosition, number : num, state : thisState, tabLevel:''};
...
var loopCount=50;
      while (!_self.isSolved() && stash.length > 0 && loopCount > 0) {
        loopCount --
        var hint=stash.pop();
        console.log(hint.tabLevel, `trying ${hint.gridIdx} = ${hint.number}`, JSON.stringify(hint))
        ...
        if (!_self.isSolved() && !isHalted) {
          ...
          thisGrid[firstUnsolvedPosition]
            .candidates.toString().split('').forEach( (num) => {
              var valueToTry = { gridIdx : firstUnsolvedPosition, number : num, state : thisState, tabLevel: hint.tabLevel + '\t'};
              stash.push(valueToTry);
          });
        } 
      }
      ...
      while ( shouldContinue  && !isHalted && !_self.isSolved()) {
        ...
        _self.unsolvedSets.forEach( (set, idx) => {
          ...
          Object.keys(candidates).forEach( (key) => {
            isDebug && candidates[key].length===1 && console.log('       ****', key, candidates[key], candidates[key].length===1);
        ...
        solvedList.forEach( (solve) => {
              isDebug && console.log('    ', '    ', '**** Setting cell ', solve.gridIdx, ' to ', solve.key);
    ...
    _self.propagate = (gridIdx, cellValue) => {
        ...
        propagationList.forEach( (gridIdx) => !isHalted && thisGrid[gridIdx].removeCandidate( cellValue ) );
    }
